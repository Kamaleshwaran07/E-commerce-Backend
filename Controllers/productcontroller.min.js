...
const productController = {
  ...
  getProductDetail: async(req,res)=>{
    ...
    try {
      ...
      res.status(200).json({message:"Successfully fetched the product details",productDetail})
      
    } catch (error) {
      res.status(500).json({message:"Server Error in getting the product details",error})
    }
  },
  cartDetails: async(req,res)=>{
    ...
    try {
      ...
      if(findUserInCart){
        const findProductInCart = findUserInCart.cart.findIndex(item=> item.productId.toString() === productId)
        console.log("Finding the Product in cart", findProductInCart)
        if(findProductInCart !== -1 ){
          findUserInCart.cart[findProductInCart].quantity += 1
        }
        else{
          findUserInCart.cart.push({productId, quantity:1, productDetails:productDet})
        }
        ...
        return res.status(200).json({message:"Quantity Updated"})
          ...
          const cartUpdate = await new Cart({
            cart: [{productDetails: productDet,
              productId:productId,
              quantity:1}],
            userId: userId
            })
          await cartUpdate.save()
          return res.status(200).json({message:"Added to the Cart", cartUpdate})
      }
