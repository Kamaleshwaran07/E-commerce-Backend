...
const productController = {
  ...
  editProductPrice: async (req, res) => {
    const userId = req.userId;
    const { productPrice, discountPercentage , productName, brandName, color, productDescription, subCategory, category } = req.body;
    ...
    try {
      ...
      const productUpdate = await Product.findByIdAndUpdate(productId, {
        productPrice, discountPercentage, brandName, productDescription, productName, color, subCategory, category
      });
  ...
  getSubCategoryWiseProducts : async(req, res)=>{
    ...
    try {
      ...
      res.status(200).json({message:"Successfully fetched products", subCategoryProducts})
  ...
  getSubcategoryProductsFromCategory : async(req, res)=>{
    ...
    try {
      const categoryProducts = await Product.find({category:category})
      console.log(categoryProducts)
      const subCategoryProducts = await categoryProducts.filter((products)=>products.subCategory === subCategory)
      console.log("SubCategory Products", subCategoryProducts)
